...
describe('ShowRetroPage', () => {
  ...
  describe('when showing retro items on desktop ', () => {
    ...
    it('has title and all its items', () => {
      ...
      expect($('.column-happy .item-text')[0].innerHTML).toContainText('the happy retro item');
      expect($('.column-happy .retro-item-add-input').attr('placeholder')).toEqual('I\'m glad that...');
      expect($('.column-meh .item-text')[0].innerHTML).toContainText('the meh retro item');
      expect($('.column-meh .retro-item-add-input').attr('placeholder')).toEqual('I\'m wondering about...');
      expect($('.column-sad .item-text')[0].innerHTML).toContainText('the sad retro item');
    ...
    describe('dialog', () => {
      ...
      describe('when dialog exists', () => {
        ...
        it('opens the dialog and hides it when the cancel button is clicked', () => {
          ...
          expect($('.archive-dialog p label')[0]).toContainText('Send action items to the team via email?');

          $('.archive-dialog__actions--cancel').simulate('touchTap');
